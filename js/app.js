...
(() => {
  ...
  function renderMetrics() {
    ...
    $("#weeklyProgress").style.width = `${clamp(weeklyKm / profile.weeklyGoal * 100, 0, 100)}%`;
    $("#weeklyGoal").textContent = `Meta adaptativa: ${numberBR(profile.weeklyGoal)} km`;
  ...
  function renderHistory() {
    ...
    const workouts = sortedWorkouts().filter(run => [run.type, run.date, run.notes, run.feeling].join(" ").toLocaleLowerCase("pt-BR").includes(term));
  ...
  function renderPlan() {
    ...
    $("#planTargetDistance").textContent = race ? `${numberBR(race.distance)} km` : `${numberBR(profile.weeklyGoal)} km`;
  ...
  function setCloudUI(mode, detail = "") {
    ...
    $("#cloudAction").textContent = mode === "online" ? "Conta conectada" : "Entrar na nuvem"; $("#cloudAction").disabled = mode === "connecting" || mode === "online"; $("#cloudSignOut").hidden = mode !== "online" && !(mode === "error" && cloudUser);
  }
  ...
  async function handleCloudUser(user) {
    ...
    cloudUnsubscribe = watchCloudState(user.uid, async remoteState => {
      if (remoteState) { applyingCloud = true; state = normalizeState(remoteState); persistLocal(); renderAll(); applyingCloud = false; setCloudUI("online"); if (firstSnapshot) showToast("Dados sincronizados com a nuvem."); firstSnapshot = false; return; }
  ...
  function bindEvents() {
    document.addEventListener("click", event => {
      ...
      if (modalButton) {
        ...
        if (id === "workoutModal") $("#workoutForm [name=date]").value = localISO();
      ...
      const deleteButton = event.target.closest("[data-delete-workout]"); if (deleteButton && window.confirm("Excluir este treino?")) { state.workouts = state.workouts.filter(item => item.id !== deleteButton.dataset.deleteWorkout); saveState("Treino excluído e plano recalculado."); renderAll(); }
    ...
    $("#historySearch").addEventListener("input", renderHistory); $("#raceHistorySelect").addEventListener("change", event => { selectedRaceHistory = event.target.value; renderRaceHistory(); });
    ...
    $("#workoutForm").addEventListener("submit", event => {
      event.preventDefault(); const form = new FormData(event.currentTarget); const durationSeconds = parseDuration(form.get("duration")); const distance = Number(form.get("distance"));
      if (!(distance > 0) || !(durationSeconds > 0)) { showToast("Confira a distância e a duração."); return; }
      state.workouts.push(normalizeWorkout({ id: uid(), date: form.get("date"), distance, durationSeconds, rpe: form.get("rpe"), type: form.get("type"), feeling: form.get("feeling"), stitch: form.get("stitch"), shoe: form.get("shoe"), notes: form.get("notes") }));
      saveState("Treino salvo. Plano e alimentação atualizados."); event.currentTarget.reset(); $("#workoutForm [name=date]").value = localISO(); $("#workoutModal").close(); selectedPlan = 0; renderAll();
    });
